...
function parkingReservation(spotsAvailable, maxTimeAllowed, listOfRequests) {
	...
	for (let i = 0; i < listOfRequests.length; i++) {
		...
		if (request.maxTimeRequested <= maxTimeAllowed && request.numberSpotsRequested <= spotsLeft) {
			requestResponse.push({
				personRequested: request.personRequested,
				status: 'Accepted',
				reason: 'Spots Available',
			});
			spotsLeft = spotsLeft - request.numberSpotsRequested;
		} else if (request.maxTimeRequested > maxTimeAllowed) {
			requestResponse.push({
				personRequested: request.personRequested,
				status: 'Denied',
